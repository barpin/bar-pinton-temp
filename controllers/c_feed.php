...
if (isset($feedname)){
    $query="SELECT * FROM categories WHERE urlname = '${feedname}' AND ctype = $allow";
...
if (isset($feedid)){
    ...
    $title=qq($link, $query)->fetch_assoc()['name'];
}

$cols=getcols($link);
$staticcategory = $category+2**2;
$notcategory = 2**2+2**5;
$votecategory = $category+2**4;
$alertcategory = $category+2**5;
$query="SELECT ${cols} FROM posts INNER JOIN textupdates ON posts.id = textupdates.post_id  WHERE posts.category & ";
$result=qq($link, $query."${staticcategory} = ${staticcategory}");
